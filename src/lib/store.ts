...
export async function getCapabilityDetailState(
  engagementId: string,
  capabilityId: string,
): Promise<CapabilityDetailState | undefined> {
  const state = await getOnboardingState(engagementId);
...
export async function getRoadmapWorkspaceState(
  ...
  return buildRoadmapWorkspaceState({
    engagement,
    initiatives: initiatives.map(mapInitiativeRecord),
  });
}
